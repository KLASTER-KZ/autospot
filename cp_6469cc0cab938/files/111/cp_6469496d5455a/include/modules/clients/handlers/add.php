...
if (!$error) {
    ...
    $insert = insert("INSERT INTO uni_clients(clients_name,clients_surname,clients_pass,clients_email,clients_phone,clients_avatar,clients_status,clients_city_id,clients_datetime_add,clients_datetime_view,clients_id_hash,clients_notifications,clients_ref_id)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", array( $_POST['user_name'],$_POST['user_surname'],$password,$_POST['user_email'],$phone, $image["name"],1,intval($_POST["city_id"]), date("Y-m-d H:i:s"), date("Y-m-d H:i:s"),$clients_id_hash,$notifications,genRefId() ));    
