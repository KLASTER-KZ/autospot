$(document).ready(function() {
    ...
    $(document).on('click','.action-edit-filter', function () {
        console.log($('.form-edit-filter')[0])
        var data_form = new FormData($('.form-edit-filter')[0]);
        $('.proccess_load').show();
        $.ajax({type: "POST",url: "include/modules/category_board/handlers/edit_filter.php",data: data_form,dataType: "html",contentType: false,cache: false,processData: false,success: function (data) { if(data == true){ location.reload();} else {$('.proccess_load').hide(); notification();} }});
