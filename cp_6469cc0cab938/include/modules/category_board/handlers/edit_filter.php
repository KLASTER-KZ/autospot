   ...
   if ($type_filter != 'input_text') {
      foreach ($_POST["value_filter"] as $action => $array) {
         foreach ($array as $id => $value) {
            ...
            if (trim($value))
               $value_filter[] = trim($value);

         }
      }

      if (!$value_filter) {
         $error[] = "Добавьте значения фильтра!";
      }
   }
      ...
      if ($_POST["value_filter"] && $type_filter != 'input_text') {
         ...
         foreach ($_POST["value_filter"] as $action => $array) {
            foreach ($array as $id => $value) {
               ...
               $find = findOne("uni_ads_filters_items", "ads_filters_items_id=?", array($id));

               if (trim($value) != "") {

                     
                  if($id_filter == 90){
                     $image = $Main->uploadedImage( ["files"=>$_FILES["image$id"], "path"=>$config["media"]["car_logo"], "prefix_name" => ""] );
                  } else if($id_filter == 309){
                     $image = $Main->uploadedImage( ["files"=>$_FILES["image$id"], "path"=>$config["media"]["car_body"], "prefix_name" => ""] );
                  } else {
                     $image = $Main->uploadedImage( ["files"=>$_FILES["image$id"], "path"=>$config["media"]["other"], "prefix_name" => ""] );
                  }
                  $rar = 79;
                  if($image["error"]){
                     $error = array_merge($error,$image["error"]);
                     $find["ads_filters_items_logo"] = 80;
                  }else{
                     if($image["name"]) $find["ads_filters_items_logo"] = $image["name"];
                  }                         

                  if ($action == "add") {
                     $insert = insert("INSERT INTO uni_ads_filters_items(ads_filters_items_id_filter,ads_filters_items_value,ads_filters_items_id_item_parent,ads_filters_items_alias,ads_filters_items_sort,ads_filters_items_logo)VALUES(?,?,?,?,?,?)", array($id_filter, clear($value), $id_item, translite($value), intval($sort), $find["ads_filters_items_logo"]));
                     $ids_delete[$insert] = $insert;
                  } elseif ($action == "edit") {
                     update("UPDATE uni_ads_filters_items SET ads_filters_items_value=?,ads_filters_items_alias=?,ads_filters_items_sort=?,ads_filters_items_logo=? WHERE ads_filters_items_id=?", array(clear($value), translite($value), intval($sort),$find["ads_filters_items_logo"], $id));
                     $ids_delete[$id] = $id;
                  }
               }
