	...
	if($_GET["search"]){
		...
		if (!preg_match("/^([0-9])+$/", $_GET["search"])) {   
			...
			if($settings["main_type_products"] == 'physical'){                                             
				...
				$query = "(clients_email LIKE '%".$_GET["search"]."%' OR ads_id LIKE '%".$_GET["search"]."%' OR clients_phone LIKE '%".$_GET["search"]."%' OR clients_name LIKE '%".$_GET["search"]."%' OR clients_surname LIKE '%".$_GET["search"]."%' OR ads_title LIKE '%".$_GET["search"]."%' OR clients_id_hash LIKE '%".$_GET["search"]."%' OR category_board_name LIKE '%".$_GET["search"]."%') and clients_status!='3' and ads_status!='8'"; 
				...
				"query_string" : {
				"query":      "*'.$_GET["search"].'*",
				"fields":     [ "ads_title", "category_board_name", "clients_name", "clients_phone", "clients_surname", "clients_email" ],
				"analyze_wildcard": "true",
				"allow_leading_wildcard": "true"
				}
                },
					...
					<form class="form-data" >
						...
						<?php
							...
							if($param_search && $config["elasticsearch"]["status"]){
								...
								$get = $Ads->getAll( array("navigation"=>false,"page"=>$_GET["page"],"output"=>$_SESSION['output_ads'],"query"=>$query, "sort"=>"ORDER By ads_datetime_add DESC", "param_search" => $param_search) );
								
								}else{
								
								$get = $Ads->getAll( array("navigation"=>true,"page"=>$_GET["page"],"output"=>$_SESSION['output_ads'],"query"=>$query, "sort"=>"ORDER By ads_datetime_add DESC", "param_search" => $param_search) );
											...
											<td class="td-actions"  style="text-align: right; min-width: 150px;" >
												...
												<?php
													...
													if ($_SERVER['REQUEST_METHOD'] === 'POST') {
														// Получение значений для вставки в таблицу
														$ads_id = $_POST['ads_id'];
														$service_id = $_POST['service_id'];
														
														// Проверка, что $ads_id не пустой и не существует в базе данных
														if (!empty($ads_id)) {
															$existingQuery = "SELECT COUNT(*) FROM uni_services_order WHERE services_order_id_ads = '$ads_id'";
															$existingResult = $conn->query($existingQuery);
															
															if ($existingResult && $existingResult->fetch_assoc()['COUNT(*)'] == 0) {
																$current_date = date('Y-m-d H:i:s');
																$validity_date = date('Y-m-d H:i:s', strtotime('+30 days'));
																$count_day = 30;
																$status = 1;
																
																// Создание и выполнение SQL-запроса для вставки данных
																$sql = "INSERT INTO uni_services_order (services_order_id_ads, services_order_time_validity, services_order_id_service, services_order_count_day, services_order_time_create, services_order_status)
																VALUES ('$ads_id', '$validity_date', '$service_id', '$count_day', '$current_date', '$status')";
													...
													$existingQuery = "SELECT COUNT(*) FROM uni_services_order WHERE services_order_id_ads = '{$value['ads_id']}'";
													$existingResult = $conn->query($existingQuery);
													$recordExists = $existingResult && $existingResult->fetch_assoc()['COUNT(*)'] > 0;
												...
												<span>
													<form method="post">
														<input type="hidden" name="ads_id" value="<?php echo $value["ads_id"]; ?>">
														<input type="hidden" name="service_id" value="2">
														<?php if (!$recordExists): ?>
														<input type="submit" name="submit" class="btn btn-gradient-01 btn-sm" style="margin-top: -5px;" value="VIP">
														<?php endif; ?>
													</form>
												</span>
												...
												<span>
													<form method="post">
														...
														<?php if (!$recordExists): ?>
														<input type="submit" name="submit" class="btn btn-gradient-03 btn-sm" style="margin-top: -5px;" value="Turbo">
														<?php endif; ?>
													</form>
												</span>
												<span>
													<form method="post">
														...
														<?php if (!$recordExists): ?>
														<input type="submit" name="submit" class="btn btn-gradient-04 btn-sm" style="margin-top: -5px;" value="Top">
														<?php endif; ?>
													</form>
												</span>
