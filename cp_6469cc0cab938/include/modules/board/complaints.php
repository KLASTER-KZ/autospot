									...
									foreach($get AS $value){
										...
										<tr>
											<td style="max-width: 350px; min-width: 250px;" >
												Жалоба на объявление <i class="la la-arrow-right"></i> <a target="_blank"  href="<?php echo $Ads->alias($getAd); ?>"><strong><?php echo $value["ads_complain_reason"]; ?></strong>: <?php echo $getAd["ads_title"]; ?></a> <i class="la la-user"></i> <a href="?route=client_view&id=<?php echo $value["ads_complain_to_user_id"]; ?>"><?php echo $getToUser["clients_name"]; ?></a>
												<div class="complaints-text" ><?php echo $value["ads_complain_text"]; ?></div>
											</td>                                     
											<td>
												<a <?php
													if (!empty($getFromUser["clients_name"])) {
														echo 'href="?route=client_view&id=' . $value["ads_complain_from_user_id"] . '"';
														} else {
														echo 'href="#"'; // или любой другой URL или действие, которое не приведет к переходу
													}
													?>><?php
														if (!empty($getFromUser["clients_name"])) {
															echo $getFromUser["clients_name"];
															} else {
															echo "Гость";
														}
													?>
