...
if (isAjax() == true) {
    ...
    if (count($error) == 0) {
        ...
        insert("INSERT INTO uni_calc ( name, actiz, nal_customs, apply, nal_import, reating_ex, customs_dec, number_transitions, customs_posh, nds, certif_secur, recycling, reg, service_escort, apply_escort, delivery_almat, operac_collecting, certif, delivery_baku, delivery_erevan, additional_expenses, delivery_bishkent)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array($name, $actiz, $nal_customs, $apply, $nal_impt, $rating_exp, $Customs_declar, $transition_number, $customs_posh, $nds, $certif_secur, $recycling, $reg, $service_escort, $apply_escort, $delivery_almat, $operac_collecting, $certif, $delivery_baku, $delivery_erevan, $additional_expenses, $delivery_bishkent));
