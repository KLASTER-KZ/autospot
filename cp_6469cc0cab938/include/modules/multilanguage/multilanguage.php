      ...
      <div class="widget has-shadow">
         <div class="widget-body">
          ...
          <ul class="nav nav-tabs nav-fill" role="tablist">
			 ...
			 <li class="nav-item <?php if($_GET["tab"] == "email_message"){ echo 'active'; } ?>">
				<a class="nav-link" id="j-drop-tab-15" href="?route=multilanguage&tab=email_message" >Шаблоны писем</a>
			 </li>
          </ul>
									...
									<form class="form-data" >
										<div class="row" >
											...
											<div class="col-lg-3" >
												<ul class="ul-a-link" >
													<?php 
														$get = getAll("SELECT * FROM uni_email_message order by id asc");
														if(count($get)){
															...
															?>
															<li><a <?php echo $active; ?> href="?route=multilanguage&id=<?php echo $value["id"]; ?>&tab=email_message"><?php echo $value["name"]; ?></a></li>
															<?php
															}
														}
													?>
													...
													if($_GET["id"]){
														...
														if(count($languages) > 0){   
															...
															foreach($languages AS $array_data){
																...
																if( file_exists( $config["basePath"] . "/lang/" . $array_data["iso"] . "/uni_email_message.php" ) ){
																	$array_lang = require $config["basePath"] . "/lang/" . $array_data["iso"] . "/uni_email_message.php";
																}else{ $array_lang = []; }
															...
															<?php
																$view = findOne("uni_email_message", "id=?", [ intval($_GET["id"]) ]);
															?>
															
															<div class="form-group">
																<label>Название</label>
																<input type="text" name="form[uni_email_message][<?php echo $array_data["iso"]; ?>][<?php echo md5("subject_" . $view["subject"]) ?>]" value="<?php if( $array_lang[ md5("subject_" . $view["subject"]) ] ){ echo removeSlash($array_lang[ md5("subject_" . $view["subject"]) ]); }else{ echo removeSlash($view["subject"]); } ?>" class="form-control">
															</div>
															...
															<?php if($view["text"]){ ?>
																<div class="form-group">
																	...
																	<textarea class="ckeditor" name="form[uni_email_message][<?php echo $array_data["iso"]; ?>][<?php echo md5("text_" . urldecode($view->text)) ?>]" class="form-control" name="email_text" ><?php if( $array_lang[ md5("text_" . urldecode($view->text)) ] ){ echo removeSlash($array_lang[ md5("text_" . urldecode($view->text)) ]); }else{ echo removeSlash(urldecode($view->text)); } ?></textarea>
																		</div>
                            ...
                            if(count($get)){
                              foreach ($get as $key => $value) {
                                ...
                                if($_GET["id"] == $value["services_tariffs_checklist_id"]){
                                 ...
                                 ?>
                                 <li><a <?php echo $active; ?> href="?route=multilanguage&id=<?php echo $value["services_tariffs_checklist_id"]; ?>&tab=user_tariffs"><?php echo $value["services_tariffs_checklist_name"]; ?></a></li>
                                 <?php
                              }
                            }
                          ?>
