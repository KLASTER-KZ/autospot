...
class Application
{
    ...
    private function CreateTransaction()
    {
        ...
        if ($found) {
            ...
            $transaction->create_time           = Format::timestamp2datetime($this->request->params['time']);
            $transaction->create_time2           = $this->request->params['time'] ?: $create_time;
            $transaction->state                 = Transaction::STATE_CREATED;
            $transaction->amount                = $this->request->amount;
            $transaction->order_id              = $this->request->account('id_order'); 
            $transaction->save(); // after save $transaction->id will be populated with the newly created transaction's id.

            // send response
            $this->response->send([
                'create_time' => $this->request->params['time'] ?: $create_time,
        ...
        switch ($found->state) {
            case Transaction::STATE_CREATED: // handle active transaction
                if ($found->isExpired()) { // if transaction is expired, then cancel it and send error
                    ...
                    $Profile = new Profile();
                    $Profile->payCallBack($found->order_id);

                    update('update uni_payme_transactions set state=?,reason=? where order_id=?', [Order::STATE_PAY_ACCEPTED, 0, $found->order_id]);

                    $params = ['id_order' => $found->order_id];
                    $order  = new Order($this->request->id);
                    $order->find($params);
    ...
    private function CancelTransaction()
    {
        ...
        switch ($found->state) {
            ...
            case Transaction::STATE_CREATED:
                ...
                update('update uni_orders_parameters set orders_parameters_state=? where orders_parameters_id_uniq=?', [Order::STATE_CANCELLED,$found->id_order]);
