               ...
               if($getUser->clients_status == 2 || $getUser->clients_status == 3){
                 ...
                 if (password_verify($user_pass.$config["private_hash"], $getUser->clients_pass)) {  
                      ...
                      if(isset($_SESSION['point-auth-location'])){
                        echo json_encode( array( "status"=>true, "location" => $_SESSION['point-auth-location'],"message" => $ULang->t("Успешно!"), "phone" => $getUser->clients_phone) );
                      }else{
                        echo json_encode( array( "status"=>true, "location" => _link( "user/".$getUser["clients_id_hash"] ),"message" => $ULang->t("Успешно!"), "phone" => $getUser->clients_phone ) );
                      }
...
if (isset($_POST["action"]) && $_POST["action"] == "reg_finish") {
    $error = [];

    $user_login = clear($_POST["user_login"]);
    $user_name = clear($_POST["user_name"]);
    $user_email = clear($_POST["user_email"]);
    ...
    if ($settings["registration_method"] == 1) {
        ...
        if (!$validatePhone['status']) {
            exit(json_encode(["status" => false, "reload" => true]));
        }
    } elseif ($settings["registration_method"] == 2) {
        if (!$user_login) {
            exit(json_encode(["status" => false, "reload" => true]));
    ...
    if (!empty($_SESSION["verify_login"][$user_login]["code"]) && $_SESSION["verify_login"][$user_login]["code"] !== $user_code_login) {
        if ($user_email) {
            exit(json_encode(["status" => false, "reload" => true]));
        } elseif ($settings["confirmation_phone"]) {
            exit(json_encode(["status" => false, "reload" => true]));
        }
    }
    ...
    if (empty($user_email)) {
        ...
        $error['user_email'] = $ULang->t("Пожалуйста, укажите Ваш адрес электронной почты");
    }

    if (empty($error)) {
        $result = $Profile->auth_reg(["method" => $settings["registration_method"], "email" => $user_email, "phone" => $user_phone, "name" => $user_name, "activation" => 1, "pass" => $user_pass]);

        $Profile->addSendPulseBook($user_email, $user_phone, $user_name, $_SESSION['langSite']['iso']);

        echo json_encode(["status" => $result["status"], "answer" => $result["answer"], "reg" => 1, "location" => _link("user/" . $result["data"]["clients_id_hash"]), "message" => $ULang->t("Успешно!")]);
...
/**
	...
	if($_POST["action"] == "reg_finish"){
      ...
      $user_login = clear( $_POST["user_login"] );
      $user_name = clear( $_POST["user_name"] );
	  $user_phone = clear( $_POST["user_phone"] );
      $user_code_login = (int)$_POST["user_code_login"];
      $user_pass = clear( $_POST["user_pass"] );
	  ...
	  if(!$user_phone){
         $error['user_phone'] = $ULang->t("Пожалуйста, укажите Ваш номер телефона");
      }
      ...
      if( !$error ){
         ...
         //Add contact SendPulse book 319646
         $Profile->addSendPulseBook($user_email, $user_phone, $user_name,$_SESSION['langSite']['iso']); 
 ...
 */  
  ...
  if($_POST["action"] == "balance_payment"){
    ...
    if($_POST["services"]){
      ...
      $services_data = array('tariffs_free-checkbox1' => $_POST["tariffs_free-checkbox1"], 'tariffs_free-checkbox2' => $_POST["tariffs_free-checkbox2"], 'tariffs_free-checkbox3' => $_POST["tariffs_free-checkbox3"], 'id_s' => $_POST["id_s"], 'id_ad' =>  $_POST["id_ad"]);
			update("UPDATE uni_clients SET clients_data=? WHERE clients_id =?", array(json_encode($services_data),$_SESSION["profile"]["id"]));
    ...
    if(!$error){
     ...
     $answer = $Profile->payMethod( $_POST["payment"], array("services" => $services,"amount" => $amount, "name" => $getUser["clients_name"], "email" => $getUser["clients_email"], "phone" => $getUser["clients_phone"], "id_order" => generateOrderId(), "id_user" => $_SESSION['profile']['id'], "action" => "balance", "title" => $static_msg["19"] . " - " . $settings["site_name"], "lang" => $_POST['lang'] ) );
