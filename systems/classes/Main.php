...
class Main{
    ...
    function assets( $assets = array(), $type = '' ){
    	...
    	if(isset($assets)){
            ...
            if(0){
                ...
                $get_cash = findOne('cash', '', [1]);
                $prefix_cash ='?'.$get_cash->data;
                if($type == 'js'){
                   foreach ($results['all'] as $link) {
                       if(strpos($link, "://") === false){
                           echo '<script src="'.$config['urlPrefix'].$config["template_folder"].'/'.$link.$prefix_cash.'"></script>';
                       }else{
                           echo '<script src="'.$link.$prefix_cash.'"></script>';
                       }
                   }
                }elseif($type == 'css'){
                   foreach ($results['all'] as $link) {
                       if(strpos($link, "://") === false){
                           echo '<link href="'.$config['urlPrefix'].$config["template_folder"].'/'.$link.$prefix_cash.'" rel="stylesheet">';
                       }else{
                           echo '<link href="'.$link.$prefix_cash.'" rel="stylesheet">';
                       }
                   }
                }else{
                   echo implode(' ', $results['all']);
    ...
    function share( $data = array() ){
        ...
        return '
		   ...
		   <a target="_blank" class="social-icon" href="https://wa.me/send?text='.$data["title"] . ' ' . $data["url"].'">
