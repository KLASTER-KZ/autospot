	...
	class Ads{
		...
		function outAdViewPrice( $param = [] ){
			...
			if( $param["data"]["ads_price_old"] ){
				...
				return '
				<div id="board-view-price_calc" class="board-view-price price-currency">
		...
		function outStatus( $service = [], $value = [] ){
			...
			if( isset($service[1]) && isset($service[2]) ){
				...
				$html .= '<div data-tippy-placement="top" title="'.$ULang->t("Вип объявление").'" > <img src="/media/others/hot.svg" style="width:22px;margin-top: -5px;" alt="fire"> </div>';
			...
			if(count($images)){
				...
				foreach ( array_slice($images, 0, $result = ($settings["img_slider"] == 0) ? 1 : (($settings["img_slider"] == 1) ? 4 : null)) as $key_image => $name ) {
					if(file_exists($config['basePath'].'/'.$config["media"]["small_image_ads"].'/'.$name)){
						$results .= '<!--18-08-2023 Correction 45-2 --><img fetchpriority="high" loading="lazy" class="image-autofocus ad-gallery-hover-slider-image" data-src="'.Exists($config["media"]["small_image_ads"],$name,$config["media"]["no_image"]).'" data-key="'.$key_image.'" alt="'.$data["ads_title"].'" src="'.Exists($config["media"]["small_image_ads"],$name,$config["media"]["no_image"]).'" >';
						}elseif(file_exists($config['basePath'].'/'.$config["media"]["big_image_ads"].'/'.$name)){
						$results .= '<!--18-08-2023 Correction 45 --><img fetchpriority="high" loading="lazy" class="image-autofocus ad-gallery-hover-slider-image " data-src="'.Exists($config["media"]["big_image_ads"],$name,$config["media"]["no_image"]).'" data-key="'.$key_image.'" alt="'.$data["ads_title"].'" src="'.Exists($config["media"]["big_image_ads"],$name,$config["media"]["no_image"]).'" >';
					}
				}
		...
		function generateAdsFilterItems($data = '') {
			...
			if (count($querys)) {
				foreach ($querys as $key => $value) {
					if($data == 'base'){
						$link = "/catalog/" . $value["ads_filters_items_value"];
					} else {
						$link = _link($settings["country_default"]) . "/avtomobili/" . $value["ads_filters_items_alias"];
					}
					$output .= "
					<div class='col-lg-1 col-md-2 col-sm-2 col-3' style='text-align: center;'><a style='color: #283544; font-weight: 500;' href='" . $link . "'>
					<img alt='car_logo' src='/media/car_logo/" . $value["ads_filters_items_logo"] . "' class='mb20' style='width: 45px;'></a></div>";
				}
			}
			...
			if (count($query)) {
				foreach ($query as $key => $value) {
					...
					$output .= "<div class='col-lg-2 col-md-3 col-sm-3 col-6 mb20'><a style='color: #283544; font-weight: 500;' href='" . $link . "'>".$ULang->t( $value["ads_filters_items_value"] , [ "table" => "uni_ads_filters", "field" => "ads_filters_items_value" ] )."</a></div>";
				}
			}
			$output .= "</div>";
			$output .= "<script>
