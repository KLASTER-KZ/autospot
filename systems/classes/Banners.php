...
function click(){
    ...
    function results( $param = [] ){
       ...
       // $Mobile_Detect->setUserAgent($_SERVER["HTTP_USER_AGENT"]);

        if ( $Mobile_Detect->isMobile() ) {
            $get = getOne("select * from  uni_advertising where advertising_banner_position=? and advertising_visible=? and advertising_lang=? and advertising_tech=? order by rand()", [$param["position_name"],1,$lang,'mob']);
        }elseif(!$Mobile_Detect->isMobile() && !$Mobile_Detect->isTablet()){
            $get = getOne("select * from  uni_advertising where advertising_banner_position=? and advertising_visible=? and advertising_lang=? and advertising_tech=? order by rand()", [$param["position_name"],1,$lang,'pk']);
        }else{
            $get = getOne("select * from  uni_advertising where advertising_banner_position=? and advertising_visible=? and advertising_lang=? and advertising_tech=? order by rand()", [$param["position_name"],1,$lang,'all']);
        }
    ...
    function positions(){
        return array(
          ...
          "catalog_base_sidebar" => array("title" => "Каталог автомобилей", "name" => "Боковая панель"),
          "catalog_base_top" => array("title" => "Каталог автомобилей", "name" => "Верхняя позиция"),
          "catalog_base_bottom" => array("title" => "Каталог автомобилей", "name" => "Нижняя позиция"), 
        );
    }
