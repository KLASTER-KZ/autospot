...
if ($settings["main_type_products"] == 'physical') {
    ...
    Router::route('catalog', function () {
        ...
        $_GET['filter'] ? $filter = $_GET['filter'] : $filter = [];

        $Seo = new Seo();
       
        $data["meta_title"] = $Seo->out(["page" => "catalog_base", "field" => "meta_title"]);
        $data["meta_desc"] = $Seo->out(["page" => "catalog_base", "field" => "meta_desc"]);

        if (count($filter)) {
            $_GET['page'] ? $page = $_GET['page'] : $page = 1;
            $found = (new CategoryBoard())->GetCar('', '', $page, 12, 0, $filter);
            $data['filter_found'] = true;
            $data_model = $found[0];
            require 'route/catalog_base.php';
        } else {
            $data['newcatalog'] = true;
            require 'route/catalog_base.php';
        }
    });

    Router::route('catalog/(.*?)', function ($riddle) {
        $Seo = new Seo();
        $riddle = urldecode($riddle);
        $riddle = str_replace(' ', '_', $riddle);
        $_GET['page'] ? $page = $_GET['page'] : $page = 1;
        $_GET['filter'] ? $filter = $_GET['filter'] : $filter = [];
        $found = (new CategoryBoard())->GetCar($riddle, '', $page, 12, 0, $filter);
        if ($found[1]) {
            $data['newcatalog'] = false;
            $data['auto']['mark'] = $riddle;

            $data["meta_title"] = $Seo->out(["page" => "catalog_base_model", "field" => "meta_title"], $data);
            $data["meta_desc"] = $Seo->out(["page" => "catalog_base_model", "field" => "meta_desc"], $data);
            
            require 'route/catalog_base.php';
        } else {
            $mark_model = explode('/', $riddle);
            $mark_model[1] = trim($mark_model[1], "#");
            $found = (new CategoryBoard())->GetCar($mark_model[0], $mark_model[1], $page, 12, $mark_model[2], $filter);
            if ($found[1] && isset($mark_model[2]) && !isset($mark_model[3])) {
                $generation = false;
                $model = true;
                $data['auto']['mark'] = $mark_model[0];
                $data['auto']['model'] = $mark_model[1];
                $data['auto']['year-from'] = $mark_model[2];
                $data['auto']['generation'] = $found[0][0]['generation'][0]['year-start'];
                $data["meta_title"] = $Seo->out(["page" => "catalog_base_model", "field" => "meta_title"], $data);
                $data["meta_desc"] = $Seo->out(["page" => "catalog_base_model", "field" => "meta_desc"], $data);
                

                require 'route/catalog_base.php';
            } else if ($found[1] && isset($mark_model[2]) && isset($mark_model[3])) {
                $complectation = true;
                $generation = false;
                $model = true;
                $key_complect = $mark_model[3];
                $data['auto']['mark'] = $mark_model[0];
                $data['auto']['model'] = $mark_model[1];
                $data['auto']['year-from'] = $mark_model[2];
                $data['auto']['generation'] = $found[0][0]['generation'][0]['year-start'];
                $data['auto']['complectation'] = $key_complect;
                ...
                $found = (new CategoryBoard())->GetCar($mark_model[0], $mark_model[1], $page, 12, 0, $filter);
                if ($found[1]) {
                    $data['auto']['mark'] = $mark_model[0];
                    $data['auto']['model'] = $mark_model[1];

                    $mark_filter = findOne("uni_ads_filters_items", "ads_filters_items_id_filter=? and ads_filters_items_alias=?", array(90,strtolower($data['auto']['mark'])));
                    $mark_filter = $mark_filter->ads_filters_items_id;

                    $model_filter = findOne("uni_ads_filters_items", "ads_filters_items_id_filter=? and ads_filters_items_alias=?", array(302,strtolower($found[0][0]['name'])));
                    $model_filter = $model_filter->ads_filters_items_id;
        ...
        if ($data["geo"] && $data["category"]) {
            ...
            if ($data["ad"]) {
                ...
                if ($data["category"]) {
                    ...
                    header("Location: " . _link("georgia/" . $alias));
