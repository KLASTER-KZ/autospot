   ...
   if(count($data)){
   	  foreach ($data as $key => $nested) {
	   	  ...
	   	  foreach ($nested as $key => $value) {
	   	  	 ...
	   	  	 if( $params["price"] == $key ){ $fields["price"] = $value ? round($value, 2) : 0; }
          ...
          if(count($error) == 0){
             ...
             if($fields["filters"]){
          	 ...
          	 $insert_id = insert("INSERT INTO uni_ads(ads_title,ads_alias,ads_text,ads_id_cat,ads_id_user,ads_price,ads_city_id,ads_region_id,ads_country_id,ads_address,ads_latitude,ads_longitude,ads_period_publication,ads_status,ads_metro_ids,ads_currency,ads_period_day,ads_datetime_add,ads_area_ids,ads_id_import,ads_import_images,ads_auto_renewal,ads_filter_tags,ads_available,ads_electron_product_links,ads_electron_product_text,ads_map_lat,ads_map_lon,ads_available_unlimitedly,ads_delivery_status,ads_delivery_weight)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",[$fields["title"],translite($fields["title"]),$fields["text"],intval($fields["category"]),intval($user_id),$fields["price"],intval($fields["geo"]["city_id"]),intval($fields["geo"]["region_id"]),intval($fields["geo"]["country_id"]),$fields["address"],$fields["latitude"],$fields["longitude"],date("Y-m-d H:i:s", strtotime($fields["datetime_add"]) + ($settings["ads_time_publication_default"] * 86400) ),0,$fields["metro"],$settings["currency_main"]["code"],$settings["ads_time_publication_default"],$fields["datetime_add"],$fields["area"],$getImport["ads_import_uniq"],$fields["images"],intval($params["auto_renewal"]),$ads_filter_tags,intval($fields["available"]),$fields["electron_product_links"],$fields["electron_product_text"],$fields["latitude"],$fields["longitude"],$fields["available_unlimitedly"],$fields["delivery_status"],$fields["delivery_weight"]]);
