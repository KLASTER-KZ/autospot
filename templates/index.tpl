...
<html lang="ka">
	<head>
		...
		<meta name='keywords' content='axali da meoradi avtomobilebi, avtolombardi, cars in georgia, my auto,my auto ge, my avto, myauto ge, myauto, myauto.ge, avtomobilebi, avto ge, auto ge, meoradi, manqanebi iafad, rustavis avto bazroba, mai avto ju, myauto.ge app, manqanebis yidva, mogos manqanebi, avto lombardi, avto papa,autopapa, მეორადი მანქანების ყიდვა, მანქანა ყიდვა, ნახმარი მანქანა, ყიდვა, იყიდება მანქანა, მანქანა, მანქანების ყიდვა გაყიდვა, მეორადი მანქანები საქართველოში, მანქანების ქირაობა, მეორადი გასაყიდი ავტომობილები, დაშლილი ავტომობილები, автопапа грузия, рустави авторынок, купить авто в грузии, авто грузия, автомобили грузия, продажа б/у, пробег, подержанные, продажа авто, продажа автомобилей, объявления, mitsubishi, honda, mazda, suzuki, subaru, audi, bmw, ford, opel, kia, hyundai, каталог, авто, автомобили, форумы, автофорум, продажа б/у, пробег, подержанные, продажа авто, продажа автомобилей, отзывы, автовладельцы, объявления, цены, продажа, toyota, nissan, mitsubishi, honda, mazda, suzuki, subaru, audi, bmw, ford, opel, kia, hyundai, каталог' />
		<!-- Facebook OpenGraph -->
		<meta property="og:locale" content="geo">
		<meta property="og:type" content="website">
		<meta property="og:url" content="https://autospot.ge/geo">
		<meta property="og:title" content="საქართველოში ავტომობილების ყიდვა გაყიდვის სანდო სერვისი">
		<meta property="og:description" content="Autospot.ge - ერთ-ერთი ყველაზე ხშირად სტუმრებადი საავტომობილო საიტია საქართველოში.">
		<meta property="og:image" content="https://autospot.ge/media/others/logo_648ea4779056c.png">
		<meta property="og:site_name" content="autospot.ge">
		...
		<!-- Correction of meta tags (duplication excluded) -->  
         <script type="application/ld+json">
         {
           ...
           "itemReviewed": {
             ...
             "address": {
                "@type": "PostalAddress",
                "streetAddress": "ბათუმი"
             },
             "telephone": "+995558003828",
             "logo": "https://autospot.ge/media/others/logo_648ea4779056c.png",
             "image": "https://autospot.ge/media/others/logo_648ea4779056c.png"
            },
           "name": "სამსახურის შეფასება",
           "ratingValue": "4.9",
           "bestRating": "5",
           "ratingCount": "24228"
         }
	...
	<body data-prefix="<?php echo $config["urlPrefix"]; ?>" data-template="<?php echo $config["template_folder"]; ?>" data-header-sticky="true" data-type-loading="<?php echo $settings["type_content_loading"]; ?>" data-page-name="<?php echo $route_name; ?>" >
		...
		<div class="container mt15" >
			...
			<div class="row" >
					...
					<?php
						foreach ($settings["home_widget_sorting"] as $widgetName) {
							...
							<?php
								...
								$currentLang = getLang();
								$query = "SELECT * FROM uni_sliders WHERE sliders_visible = ? AND (sliders_lang = ? OR sliders_lang = 'all') ORDER BY RAND()";
								$data["sliders"] = getAll($query, [1, $currentLang]);
				...
				<?php
					$currentLang = getLang();
					$query = "SELECT * FROM uni_sliders WHERE sliders_visible = ? AND (sliders_lang = ? OR sliders_lang = 'all')";
					$data["sliders"] = getAll($query, [1, $currentLang]);
					if( count($data["sliders"]) ){
					?>
					<div class="load-sliders-wide mb25 " >
				...
				<?php
					...
					if ($widgetName) {
						...
						$optionValue = $settings["index_out_content_method"];
						
						switch ($optionValue) {
							case 1:
							// Case 1: Show VIP and Recommended
							$data["vip"] = $Ads->getAll(["query" => "ads_status='1' and clients_status IN(0,1) and ads_period_publication > now() and ads_vip='1' order by rand() limit ".$settings["index_out_col_ads_vip"]."", "param_search" => $param_search, "output" => '".$settings["index_out_col_ads_vip"]."']);
							...
							?>
							<div class="mb25">
								<div class="mb25 title-and-link h3"> <strong><?php echo $ULang->t( "Горячие предложения" ); ?></strong> 
									
									<span class="d-none d-md-block"  style="float: right;font-size:16px; margin-top: 5px;">
										
										<img src="/media/others/hot.svg" style="width:22px;margin-top: -5px;" alt="fire">
							...
							$data["vip_second"] = $Ads->getAll(["query" => "ads_status='1' and clients_status IN(0,1) and ads_period_publication > now() and ads_vip='0' ORDER BY ads_id  DESC limit ".$settings["index_out_col_ads"]."", "param_search" => $param_search, "output" => '".$settings["index_out_col_ads"]."']);
							
							if ($settings["main_type_products"] == 'physical') {
								if ($_SESSION["geo"]["alias"]) {
									$data["vip_link_second"] = _link($_SESSION["geo"]["alias"] . "/vip");
									} else {
									$data["vip_link_second"] = _link($settings["country_default"] . "/vip");
								}
								} else {
								$data["vip_link_second"] = _link("vip");
							}
							
							if ($data["vip_second"]["count"]) {
							?>
							<div class="mb25">
								<div class="mb25 title-and-link h3"> <strong><?php echo $ULang->t("Рекомендации для вас"); ?></strong></div>
								<div class="row no-gutters gutters10">
									<?php
										foreach ($data["vip_second"]["all"] as $key => $value) {
											include $config["template_path"] . "/include/home_ad_grid.php";
										}
									?>
								</div>
							...
							<div class="h3 mb20">
				<strong><?php echo $ULang->t("Выбор автомобиля по марке"); ?></strong>
				<a class="content_toggle" style="text-decoration: none;" href="#">   <?php echo $ULang->t("Все марки"); ?> <i class="la la-angle-down"></i></a><a href="<?php echo _link($settings["country_default"]) ?>/avtomobili">
					<button style="float: right;" class="d-none d-xl-block btn btn-dark submit-filter-form">
						<span class="action-load-span-start">
							<span class="spinner-border spinner-border-sm button-ajax-loader" role="status" aria-hidden="true"></span>
						</span>
						<span class="action-load-span-end"><?php echo $ULang->t("Расширенный поиск"); ?></span>
					</button>
				</a>
			</div>
			<div class="content_block hide">
				<?php echo $Ads->generateAdsFilterItems($data); ?>
			</div>
							...
							case 3:
							// Case 3: Show only Recommended
							$data["vip_second"] = $Ads->getAll(["query" => "ads_status='1' and clients_status IN(0,1) and ads_period_publication > now() ORDER BY ads_id  DESC limit ".$settings["index_out_col_ads"]."", "param_search" => $param_search, "output" => '".$settings["index_out_col_ads"]."']);
				...
				<div class="d-none d-xl-block mt20">          
