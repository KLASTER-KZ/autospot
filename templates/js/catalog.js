$(document).ready(function () {
  ...
  $(document).on('change', '.filters_base-box .mark input', function (e) {


    $.ajax({
      type: "POST", url: url_path + "systems/ajax/ads.php", data: "mark="+ e.target.attributes['data-model'].value + "&action=get_filters_base_models", dataType: "json", cache: false, success: function (data) {
        var html = ''
        html += `
        <div class="uni-select filter_base model" data-status="0">
        ...
        data.forEach((element, index) => {
          html += `
									<div class="custom-control custom-radio" onclick="">
										<input type="radio" class="custom-control-input" name="filter[model]" id="model${index}"
										value="${element['name']}" >
										<label class="custom-control-label" for="model${index}">
                    ${element['name']}
										</label>
									</div>
          `
        ...
        `
        $('.filter_base.model').remove()
        $(e.target).parent().parent().parent().after(html)

        document.querySelectorAll('.filters_base .uni-select-list').forEach((el, index) => {
          el.querySelectorAll('.filters_base .uni-select-list input').forEach(input => {
            input.removeEventListener('change', () => {})
          })
        })
        if(document.querySelector('.filters_base .uni-select-list')){
          document.querySelectorAll('.filters_base .uni-select-list').forEach((el, index) => {
            el.querySelectorAll('.filters_base .uni-select-list input').forEach(input => {
              if(input.checked) {
                  document.querySelectorAll('.uni-select-name span')[index].innerHTML = input.value
                }
  ...
  $(document).on("click", ".catalog-ad-view", function (e) {
    ...
    $.ajax({
      type: "POST", url: url_path + "systems/ajax/ads.php", data: "view=" + $(this).data("view") + "&action=catalog_view", dataType: "html", cache: false, success: function (data) {
        location.reload();
      }
    });
  ...
  function catalogLoadAds(_page = 1, _this_button = null, _scroll = false, fix = false) {
    ...
    $.ajax({
      type: "POST", url: url_path + "systems/ajax/ads.php", data: formFilter() + "&page=" + _page + "&fix=" + fix + "&action=load_catalog_ads", dataType: "json", cache: false, success: function (data) {
        ...
        if (data['catalog_fix']) {
          $(".catalog-results").append('<div class="load-page' + _page + ' col-lg-12" ></div>' + data["content"]);
          catalogLoadAds(currentCountPage, null, false, true);
        }else{
  ...
  function catalogBaseLoadAds(_page = 1, _this_button = null, _scroll = false, fix = false) {
    ...
    mark = decodeURI(mark[mark.length - 1]).replaceAll(' ','_').replaceAll('#','') 
    $.ajax({
      type: "POST", url: url_path + "systems/ajax/ads.php", data: formFilter() + "&page=" + _page + "&fix=" + fix + "&mark="+ mark + "&action=load_base_catalog_ads", dataType: "json", cache: false, success: function (data) {
        console.log(data);
        if (parseInt(data['count'])) $('.catalog-count-ad').html(data['count']); else $('.catalog-count-ad').html('');
        $(".preload, .preload-scroll").hide();
        ...
        $(".action-catalog-load-ads").hide();
        if (data['catalog_fix']) {
          $(".catalog-base-results").append('<div class="load-page' + _page + ' col-lg-12" ></div>' + data["content"]);
          catalogBaseLoadAds();(currentCountPage, null, false, true);
        }else{
          $(".catalog-base-results").append('<div class="load-page'+_page+' col-lg-12" ></div><div class="row no-gutters gutters10" style="display: none;" >'+data["content"]+'</div>');
        }
        $('.load-page' + _page).next().fadeIn('slow');
  ...
  $(document).on('click', '.action-catalog-load-ads > button', function () {
    ...
    catalogBaseLoadAds(currentCountPage, $(this), true);
  ...
  if ($("body").data("type-loading") == 2) {
    ...
    $(window).scroll(function () {
      ...
      if (($(document).scrollTop() + 100) >= $(".catalog-results").height()) {
      ...
      if (($(document).scrollTop() ) >= $(".catalog-base-results").height()) {
        if (loadingAdScroll == true) {
 
          currentCountPage = currentCountPage + 1;

          catalogBaseLoadAds(currentCountPage, null, false); 
