$(document).ready(function () {
   ...
   $(document).on('click','.action-reg-verify', function (e) {  
	...
	$(document).on('click','.action-auth-send-create', function (e) {  
      ...
      $.ajax({type: "POST",url: url_path + "systems/ajax/profile.php",data: "user_login=" + $(".auth-block-tab-auth input[name=user_login]").val() + "&user_pass=" + $(".auth-block-tab-auth input[name=user_pass]").val() + "&save_auth=" + $(".auth-block-tab-auth input[name=save_auth]").val() + "&captcha=" + $(".auth-block-tab-auth input[name=captcha]").val() + "&action=check_auth",dataType: "json",cache: false,success: function (data) { 
         ...
         if( data["status"] == true ){
            if(data["phone"]){
               document.querySelector('.auth_create').innerHTML = data["message"]
               document.querySelector('.ads-create-publish').style = "display:block"
               document.querySelector('.ads-create-publish').click()
            }else{
               document.querySelector('.display_phone_create').style = ""
               document.querySelector('.auth_create').style = "display:none;"
            }
   ...
   $(document).on('click', '.action-reg-finish-create', function (e) {
