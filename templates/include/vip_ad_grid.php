...
<div class="item-grid" title="<?php echo $value["ads_title"]; ?>" >
	...
	<div class="item-grid-img thumbs" >
		...
		<a href="<?php echo $Ads->alias($value); ?>" style="color: #fff;">
			<?php echo $Ads->adActionFavorite($value, "catalog", "item-grid-favorite"); ?>
			<div class="caption">
				<span class="title"><?php
					$datas = custom_substr($value["ads_title"], 35, "...");
					$partss = explode(" ", $datas);
					
					for ($is = 0; $is < min(count($partss), 100); $is++) {
						echo '<span class="tag' . $is . '"> '.$ULang->t($partss[$is], ["table" => "uni_ads_filters", "field" => "ads_filters_items_value"]).' </span>';
					}
				?>
				</span>
