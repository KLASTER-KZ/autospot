...
<div class="catalog-more-filter s catalog-more-filter-action d-none">
	...
	<?php 
		if($data["category"]["category_board_id"]){
			...
			?>
			<div class="catalog-list-options toggle-list-options d filter-items" id="usdPriceBlock_l">
				<div class="filter-input">
					<div class="wb">
						<span></span>
			...
			<div class="catalog-list-options toggle-list-options d filter-items" id="lariPriceBlock_l" style="display: none;">
				<div class="filter-input">
					<div class="wb">
						<span></span>
		...
		?>
		<div class="catalog-list-options toggle-list-options d filter-items" id="usdPriceBlock_m">
			<div class="filter-input">
				<div class="wb">
					<span></span>
					<input type="text" placeholder=" <?php echo $ULang->t("Цена от"); ?> $"  name="filter[price_usd][from]" value="<?php if($data["param_filter"]["filter"]["price_usd"]["from"]) echo round($data["param_filter"]["filter"]["price_usd"]["from"],2); ?>" />
		...
		<div class="catalog-list-options toggle-list-options d filter-items" id="lariPriceBlock_m" style="display: none;">
			<div class="filter-input">
				<div class="wb">
					<span></span>
					<input type="text" placeholder=" <?php echo $ULang->t("Цена от"); ?> ₾" name="filter[price][from]" value="<?php if($data["param_filter"]["filter"]["price"]["from"]) echo round($data["param_filter"]["filter"]["price"]["from"],2); ?>" />
...
<div class="catalog-more-filter s <?php if($data["count_filters"] > 4){ ?>catalog-more-filter-action <?php } ?>" >
	...
	<div class="modal-search-geo-container d" >
		<input type="text" placeholder="<?php echo $ULang->t("Город или регион"); ?>" class="form-control modal-search-geo-input w" style="background:#fff; padding: 11px;" value="<?php echo $ULang->t($Geo->change()["name"], [ "table"=>"geo", "field"=>"geo_name" ] ); ?>" id="searchInput">
		<span id="clearInput" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); cursor: pointer;"><svg width="24px" height="24px" viewBox="-2.4 -2.4 28.80 28.80" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"><rect x="-2.4" y="-2.4" width="28.80" height="28.80" rx="14.4" fill="#ffffff" strokewidth="0"></rect></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M12 12L7 7M12 12L17 17M12 12L17 7M12 12L7 17" stroke="#F47B28" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></path></g></svg></span>
		<div class="modal-search-geo-results" style="display: none;"></div>
	...
	<?php echo $Filters->load_filters_catalog( $data["category"]["category_board_id"] , $data["param_filter"], "filters_catalog" ); ?>
		...
		if($data["category"]["category_board_id"]){
			...
			<div class="catalog-list-options toggle-list-options d filter-items" id="usdPriceBlock_f">
				<div class="filter-input">
					<div class="wb">
						<span></span>
						<input type="text" placeholder=" <?php echo $ULang->t("Цена от"); ?> $"  name="filter[price_usd][from]" value="<?php if($data["param_filter"]["filter"]["price_usd"]["from"]) echo round($data["param_filter"]["filter"]["price_usd"]["from"],2); ?>" />
			...
			<div class="catalog-list-options toggle-list-options d filter-items" id="lariPriceBlock_f" style="display: none;">
				<div class="filter-input">
					<div class="wb">
						<span></span>
						<input type="text" placeholder=" <?php echo $ULang->t("Цена от"); ?> ₾" name="filter[price][from]" value="<?php if($data["param_filter"]["filter"]["price"]["from"]) echo round($data["param_filter"]["filter"]["price"]["from"],2); ?>" />
		...
		?>
		<div class="catalog-list-options toggle-list-options d filter-items" id="usdPriceBlock_y">
			<div class="filter-input">
				<div class="wb">
					<span></span>
					<input type="text" placeholder=" <?php echo $ULang->t("Цена от"); ?> $"  name="filter[price_usd][from]" value="<?php if($data["param_filter"]["filter"]["price_usd"]["from"]) echo round($data["param_filter"]["filter"]["price_usd"]["from"],2); ?>" />
		...
		<div class="catalog-list-options toggle-list-options d filter-items" id="lariPriceBlock_y" style="display: none;">
			<div class="filter-input">
				<div class="wb">
					<span></span>
					<input type="text" placeholder=" <?php echo $ULang->t("Цена от"); ?> ₾" name="filter[price][from]" value="<?php if($data["param_filter"]["filter"]["price"]["from"]) echo round($data["param_filter"]["filter"]["price"]["from"],2); ?>" />
...
<script src="/templates/js/filters_show.js?=v88"></script>

<style>
	.css-rw3255{background: #dbdbdb;}	
...
<script>
	$(document).ready(function() {
		// Проверка наличия текста в поле перед добавлением обработчика события
		if ($('#searchInput').val().trim() !== '') {
			$('#clearInput').show(); // Показывать крестик только если поле не пусто
		}
		
		// Очистка поля ввода по нажатию на крестик
		$('#clearInput').on('click', function() {
			$('#searchInput').val('');
			$(this).hide(); // Скрытие крестика после очистки поля
		});
		
		// Добавление обработчика события для проверки текста при изменении содержимого поля
		$('#searchInput').on('input', function() {
			if ($(this).val().trim() !== '') {
				$('#clearInput').show(); // Показывать крестик при вводе текста
				} else {
				$('#clearInput').hide(); // Скрытие крестика, если поле пусто
			}
		});
	});
	</script>		