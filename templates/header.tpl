...
<header class="header-wow d-none d-lg-block" >
	...
	<div class="header-wow-sticky" >
			...
			<?php if($route_name == 'shop'){ ?>
				...
				<div class="container" >
					...
					<div class="row" >
						...
						<div class="col-lg-2 col-md-2 col-sm-2" style="max-width:165px;">
						...
						<div class="header-flex-box-3">
							...
							<?php if($settings["visible_lang_site"]){ ?>
								<div class="header-wow-top-lang"   translate="no" >
									...
									<img style="width: 20px;border-radius: 50px; margin-left: 6px;"src="/media/others/<?php echo getLang(); ?>.webp" alt="<?php echo $_SESSION['langSite']['name']; ?>">  
						...
						<div class="header-flex-box-1">
							<div class="row" style="float: right;padding: 12px;">
								<div style="cursor: pointer;" id="btnUSD" class="currency-button">
									<div style="padding: 0px;border: 1px solid #efefef; border-radius: 3px 0 0 3px;"> <strong>   $  </strong> </div>
								</div>
								<div style="cursor: pointer;" id="btnLARI" class="currency-button">
									<div style=" padding: 0px;border: 1px solid #efefef; border-radius: 0 3px 3px 0;"> <strong> ₾  </strong> </div>
								</div>
							</div>
						</div>
					...
					<div class="col-lg-12 d-none d-lg-block " style="padding: 0px;margin-top:17px;z-index: -1;">
						
					</div>
				<?php } ?>
				<?php include $config["template_path"] . "/menu.tpl"; ?>
		...
		<div class="mt60">
			<?php if ($route_name == "inde") { ?>
				<div class="header-wow-top css-463gj4">
					<div class="container css-qwr2356">
						<div class="row">
							<div class="col-lg-9 col-9 text-left" style="padding: 3px;">
								<div class="header-wow-top-list <?php if ($_SESSION['cp_auth'][$config["private_hash"]] && $_SESSION["cp_control_tpl"]) {
									echo 'header-wow-top-list-admin';
								} ?>">
								<?php if ($_SESSION['cp_auth'][$config["private_hash"]] && $_SESSION["cp_control_tpl"]) {
									echo '<span class="header-wow-top-list-admin-edit open-modal" data-id-modal="modal-edit-site-menu"  >' . $ULang->t("Изменить") . '</span>';
								} ?>
								<?php
									if (count($settings["frontend_menu"])) {
										...
										?>
										<a style="color: #747474;" href="<?php echo $link; ?>" <?php echo $target; ?>><?php echo $ULang->t($value["name"]); ?></a>
										<?php
										}
									}
								?>
	...
	<?php if($route_name == "index" ){ ?>
		<div class="d-block d-lg-none">
			<div class="row no-gutters gutters12 col-lg-12 " style="padding-right: 0px; padding-left: 0px; margin-top: 105px;border-bottom: 10px solid #f2f2f2;border-top: 10px solid #f2f2f2;">
				<?php
	...
	<header class="header-wow-mobile d-block d-lg-none <?php if( $route_name == "catalog" || $route_name == "index" ){ echo ''; }else{ echo 'height55'; } ?>" <?php if($route_name == "map"){ echo 'style="margin-bottom:0px;"'; } ?> >
		...
		<div class="header-wow-mobile-sticky" >
			<div class="header-wow-mobile-top parents-ajax-live-search" >
				...
				<div class="container" >
					<div class="header-flex-box" >   
							...
							<?php if( $route_name == "catalog" ){ ?>
								...
								<div class="toolbar-link" >
									...
									<div class="toolbar-link-title-icon" >
										...
										<span class="" ><?php echo $ULang->t("Меню"); ?></span>
										</div>
						...
						<div class="header-flex-box-mobile-3" >
							...
							<?php if($route_name == "catalog" || $route_name == "index" || $route_name == "ad_create"){ ?>
										...
										<div class="header-wow-top-lang" translate="no">
											<div class="toolbar-dropdown dropdown-click">
												<span class="header-wow-top-lang-name" >
													...
													<img style="width: 30px;border-radius: 50px;"src="/media/others/<?php echo getLang(); ?>.webp" alt="Icon language">
				...
				<div class="container" style="background: #f2f2f2; padding: 10px;">			
					<div class="modal-search-geo-container">
						
						<?php if($route_name == "catalog" || $route_name == "index" || $route_name == "shops" || $route_name == "shop" || $route_name == "map" || $route_name == "ad_view"){ ?>
									...
									<?php if($settings["main_type_products"] == 'physical'){ ?>
										<form class="form-ajax-live-search" method="get" <?php if($route_name != "shops" && $route_name != "shop"){ ?> action="<?php if($route_name != 'map'){ echo $_SESSION["geo"]["alias"] ? _link($_SESSION["geo"]["alias"]) : _link($settings["country_default"]); }else{ echo $_SESSION["geo"]["alias"] ? _link('map/'.$_SESSION["geo"]["alias"]) : _link('map/'.$settings["country_default"]); } ?>" <?php } ?> >
											...
											<div class="header-wow-mobile-sticky-search" >
												...
												<button name="search" class="header-wow-mobile-sticky-search-icon"><i class="las la-search" style="transform: scaleX(-1);"></i><div class="input-separator"></div></button>
												
												<input style="border-radius: 5px;font-weight: 500;" type="text" name="search" class="ajax-live-search" autocomplete="off" placeholder="<?php if($route_name == "shops"){ echo $ULang->t("Поиск по магазинам"); }elseif($route_name == "shop" && $data["tariff"]['services']['search_shop']){ echo $ULang->t("Поиск по объявлениям магазина"); }elseif($route_name == "blog"){ echo $ULang->t("Поиск по блогу"); }else{ echo $ULang->t("Поиск"); } ?>" value="<?php echo clear($_GET["search"]); ?>" >
												
													<div class="sticky-search-control-geo mobile-fixed-menu_catalog_filters-open" ><i class="las la-sliders-h" style="font-size: 17px;"></i> <span style="font-weight: 500;"><?php echo $ULang->t("Параметры"); ?><span></div>
											<?php if($route_name != "ad_view"){ ?>		<?php } ?>
													</div>
													<?php if($route_name == "shop"){ ?>
														<input type="hidden" name="id_s" value="<?php echo $data["shop"]["clients_shops_id"]; ?>" >
													<?php } ?>
